...
export const runBacktestHandler = async (req, res) => {
  try {
    ...
    if (!code || !stocks?.length || !startDate || !endDate) {
...
export const getBacktest = async (req, res) => {
  try {
    const { userId } = req.user; // Securely scope by userId
    const backtest = await Backtest.findOne({ backtestId: req.params.id, userId });
    if (!backtest) return res.status(404).json({ error: 'Backtest not found or unauthorized' });
    res.json(backtest);
  } catch (err) {
...
/**
 * getBacktests - Retrieves past simulation lists for the authenticated user.
...
export const getBacktests = async (req, res) => {
  try {
    ...
    const filter = { userId }; // Strict ownership filter
...
/**
 * clearBacktests - Deletes all backtests run by the user.
